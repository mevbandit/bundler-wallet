...
task(
    ...
    async (_, { config }, runSuper) => {
        ...
        await runSuper();
        console.log('Compiling Yul files...');
            ...
            try {
                ...
                if (typeof output.errors !== 'undefined') {
                    ...
                    if (error.severity === 'warning') {
                        console.log(chalk.yellow('[WARNING]', error.formattedMessage));
                    } else if (error.severity === 'error') {
                        console.log(chalk.red('[ERROR]', error.formattedMessage));
                ...
                const savePath = path.join(
                    config.paths.artifacts, 'contracts', `${contractName}.json`
                );
                fs.writeFileSync(savePath, JSON.stringify({bytecode, compilerStandardJsonInput: input}, null, 4));
            } catch (err) {
                console.error(err);
                process.exit(1);
            }
